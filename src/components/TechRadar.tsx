    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm">
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
          <div>
            <span className="font-mono text-xs uppercase tracking-wider text-teal-800 font-semibold">
          ...
          <span className="font-sans text-xs text-slate-500 font-medium">Production-Grade Standards</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
          {techCapabilities[activeCategory].items.map((tech) => (
            <div
              key={tech}
              className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50/70 p-3.5 transition-all hover:border-teal-500/40 hover:bg-slate-50"
            >
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-teal-50 text-teal-700 font-mono text-xs border border-teal-200">
                <CheckCircle2 className="h-4 w-4" />
              </div>
              <div>
                <span className="text-sm font-semibold text-slate-900">{tech}</span>
                <p className="font-sans text-[11px] text-slate-500 font-medium">Enterprise Certified</p>
              </div>
            </div>
          ))}
        </div>
