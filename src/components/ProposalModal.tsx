...
export default function ProposalModal({ isOpen, onClose }: ProposalModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-bg/85 p-4 backdrop-blur-md animate-in fade-in duration-200"
      onClick={(e) => {
    ...
    >
      <div
        ref={modalRef}
        className="relative w-full max-w-xl max-h-[90vh] overflow-y-auto rounded-2xl border border-lineLight bg-bgRaised p-6 md:p-8 shadow-glass animate-in zoom-in-95 duration-200"
      >
        {/* Close Button */}
        <button
          type="button"
          ...
          className="absolute right-4 top-4 flex h-9 w-9 items-center justify-center rounded-xl border border-line bg-card text-muted hover:border-brand-500 hover:text-ink transition-colors"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        {/* Modal Header */}
        <div className="mb-6">
          <div className="inline-flex items-center gap-2 rounded-full border border-brand-500/30 bg-brand-500/10 px-3 py-1 font-mono text-[11px] text-brand-300 mb-2">
            <Sparkles className="h-3 w-3 text-brand-400" />
            Direct Proposal Channel
          </div>
          <h2 id="proposal-modal-title" className="text-xl md:text-2xl font-display font-bold text-ink">
            Request an Engineering Proposal
          </h2>
          <p className="mt-1 text-xs md:text-sm text-muted">
            Share your project parameters with the BIGWI HOUSE division. Our engineering team will review requirements and structure a formal capability &amp; architecture proposal.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="pm-name" className="mb-1.5 block text-xs font-mono text-muted">
                Full Name *
              </label>
              <input
                id="pm-name"
            ...
            <div>
              <label htmlFor="pm-email" className="mb-1.5 block text-xs font-mono text-muted">
                Work Email *
              </label>
              <input
                id="pm-email"
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="pm-org" className="mb-1.5 block text-xs font-mono text-muted">
              ...
              <input
                ...
                placeholder="e.g. Bank, NGO, or Startup"
                className="w-full rounded-xl border border-line bg-card px-3.5 py-2.5 text-sm text-ink placeholder:text-subtle focus:border-brand-500 focus:outline-none transition-colors"
              />
            </div>

            <div>
              <label htmlFor="pm-need" className="mb-1.5 block text-xs font-mono text-muted">
              ...
              >
                ...
                <option value="SaaS Product Incubation">SaaS Product Incubation</option>
              </select>
            </div>
          </div>

          <div>
            <label htmlFor="pm-scope" className="mb-1.5 block text-xs font-mono text-muted">
          ...
          {status === 'success' && (
            <div className="flex items-center gap-3 rounded-xl border border-brand-500/40 bg-brand-500/10 p-3.5 text-xs text-brand-300">
              ...
              <div>
                ...
                <p className="text-muted">A senior solutions architect from BIGWI HOUSE will follow up within 24 hours.</p>
              </div>
            </div>
          )}
